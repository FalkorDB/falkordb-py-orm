...
class Company:
    ...
    employees: List["Employee"] = relationship(
        "EMPLOYS", direction="OUTGOING", target="Employee", cascade=True
    )
...
class Employee:
    ...
    direct_reports: List["Employee"] = relationship(
        "MANAGES", direction="OUTGOING", target="Employee"
    )
...
class Project:
    ...
    team_members: List[Employee] = relationship("WORKS_ON", direction="INCOMING", target=Employee)
...
class User:
    ...
    connections: List["User"] = relationship("CONNECTED", direction="BOTH", target="User")
...
class Customer:
    ...
    shipping_address: Optional[Address] = relationship(
        "HAS_SHIPPING_ADDRESS", target=Address, cascade=True, lazy=False
    )

    billing_address: Optional[Address] = relationship(
        "HAS_BILLING_ADDRESS", target=Address, cascade=True
    )
...
def inspect_metadata():
    ...
    print(
        f"  Get 'friends' relationship: {friends_rel.relationship_type if friends_rel else 'None'}"
    )
    print()
