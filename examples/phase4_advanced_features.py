...
class PersonRepository(Repository[Person]):
    ...
    @query("MATCH (p:Person)-[:KNOWS]->(f:Person) WHERE p.name = $name RETURN f", returns=Person)
    def find_friends_of(self, name: str) -> List[Person]:
        """Find all friends of a person by name."""
        pass

    @query("MATCH (p:Person) WHERE p.age > $min_age AND p.age < $max_age RETURN p", returns=Person)
    ...
    @query("MATCH (p:Person) WHERE p.email CONTAINS $domain RETURN p", returns=Person)
    def find_by_email_domain(self, domain: str) -> List[Person]:
        """Find people with email from specific domain."""
        pass

    @query("MATCH (p:Person) RETURN count(DISTINCT p.age) as unique_ages", returns=int)
