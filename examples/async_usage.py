...
async def basic_async_operations():
    ...
    pool = BlockingConnectionPool(
        max_connections=16, timeout=None, decode_responses=True, host="localhost", port=6379
    )
...
async def concurrent_operations():
    ...
    people = [
        Person(name=f"Person{i}", email=f"person{i}@example.com", age=20 + i) for i in range(5)
    ]

    # Save all concurrently using asyncio.gather
    saved_people = await asyncio.gather(*[person_repo.save(person) for person in people])
    ...
    results = await asyncio.gather(
        person_repo.count(), person_repo.avg("age"), person_repo.find_by_age_greater_than(22)
    )
