...
pytestmark = pytest.mark.skipif(
    not FALKORDB_AVAILABLE, reason="FalkorDB not installed or not available"
)
...
def db():
    ...
    try:
        db_instance = FalkorDB(host="localhost", port=6379)
        # Test connection
        db_instance.connection_pool.get_connection().ping()
        return db_instance
...
class TestPagination:
    ...
    def test_basic_pagination(self, person_repo):
        ...
        for i in range(15):
            person_repo.save(Person(name=f"Person{i}", email=f"person{i}@page.com", age=20 + i))
    ...
    def test_page_navigation(self, person_repo):
        ...
        # Create test data
        for i in range(10):
            person_repo.save(Person(name=f"Nav{i}", email=f"nav{i}@page.com", age=20 + i))
