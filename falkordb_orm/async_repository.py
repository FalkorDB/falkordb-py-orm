"""Async repository pattern for entity data access."""
...
class AsyncRepository(Generic[T]):
    """
    Async repository for entity data access.
    
    Provides async CRUD operations and query methods for entities.
    ...
    def __init__(self, graph: Any, entity_class: Type[T]):
        ...
        Args:
            graph: FalkorDB async graph instance (from falkordb.asyncio)
        ...
        # Get metadata synchronously (metadata is not async)
        from .mapper import EntityMapper
        sync_mapper = EntityMapper()
        self.metadata: EntityMetadata = sync_mapper.get_entity_metadata(entity_class)
        self._query_spec_cache: Dict[str, QuerySpec] = {}
        
        # Initialize async relationship manager
        from .async_relationships import AsyncRelationshipManager
        self.relationship_manager = AsyncRelationshipManager(
            graph=graph,
            mapper=self.mapper,
        ...
        try:
            ...
            if result.result_set:
                ...
                if self.metadata.relationships:
                    await self.relationship_manager.save_relationships(
    ...
    async def find_by_id(self, entity_id: Any, fetch: Optional[List[str]] = None) -> Optional[T]:
        """
        Find entity by ID.
        ...
        try:
            ...
            if fetch:
                ...
                record = result.result_set[0]
                return await self.mapper.map_with_relationships(
                    record, self.entity_class, fetch
                )
            else:
                ...
                record = result.result_set[0]
                return await self.mapper.map_from_record(record, self.entity_class)
            
        except Exception as e:
            raise QueryException(f"Failed to find entity by ID: {e}") from e
    
    async def find_all(self, fetch: Optional[List[str]] = None) -> List[T]:
        """
        Find all entities.
        ...
        try:
            ...
            if fetch:
                ...
                for record in result.result_set:
                    entity = await self.mapper.map_with_relationships(
                        record, self.entity_class, fetch
                    )
                ...
                entities: List[T] = []
                for record in result.result_set:
                    entity = await self.mapper.map_from_record(record, self.entity_class)
                    entities.append(entity)
    ...
    async def find_all_by_id(self, ids: Iterable[Any]) -> List[T]:
        """
    ...
    async def exists_by_id(self, entity_id: Any) -> bool:
        ...
        """
        try:
            cypher, params = self.query_builder.build_exists_by_id_query(
                self.metadata, entity_id
            )
    ...
    async def sum(self, property_name: str) -> float:
        """
    ...
    async def avg(self, property_name: str) -> float:
        """
    ...
    async def min(self, property_name: str) -> Any:
        """
    ...
    async def max(self, property_name: str) -> Any:
        """
    ...
    async def delete_by_id(self, entity_id: Any) -> None:
        ...
        """
        try:
            cypher, params = self.query_builder.build_delete_by_id_query(
                self.metadata, entity_id
            )
    ...
    async def delete_all(self, entities: Optional[Iterable[T]] = None) -> None:
        """
    ...
    def _create_derived_query_method(self, method_name: str) -> Callable:
        ...
        Returns:
            Callable that executes the derived query
        """
        async def derived_query_method(*args: Any, **kwargs: Any) -> Any:
